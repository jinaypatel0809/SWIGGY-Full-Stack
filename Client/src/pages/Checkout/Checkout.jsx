...
function Checkout() {
  ...
  return (
    <main className="mx-auto max-w-[900px] px-4 py-12 sm:px-6"><h1 className="text-3xl font-semibold">Checkout</h1>
      ...
      <section className="mt-6 rounded-2xl border border-zinc-200 p-5 dark:border-zinc-800"><h2 className="text-xl font-semibold">Payment</h2><div className="mt-4 grid gap-3 sm:grid-cols-2"><button onClick={() => setError('Online payment will be enabled after Razorpay setup.')} className="flex justify-center gap-2 rounded-xl border py-4"><CreditCard /> Online Payment</button><button onClick={sendCodOtp} className="flex justify-center gap-2 rounded-xl bg-[#e23744] py-4 font-semibold text-white"><Banknote /> Cash on Delivery</button></div>{otpSent && <div className="mt-4 flex gap-3"><input value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, ''))} className="admin-input" placeholder="COD OTP" /><button onClick={confirmOrder} className="shrink-0 rounded-lg bg-green-600 px-5 font-semibold text-white">Confirm order</button></div>}{message && <p className="mt-3 text-sm text-green-700">{message}</p>}{error && <p className="mt-3 text-sm text-[#e23744]">{error}</p>}</section>
    </main>
  )
}
